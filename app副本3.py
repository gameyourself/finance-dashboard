...
def call_deepseek(prompt: str, system_prompt: str = "你是一个擅长家庭财务建模与建议输出的助手。"):
...
def render_compare_table(current, previous):
    ...
    st.caption(f"当前列“{current_month}”代表本月，列“{prev_month}”代表上个月。")
...
def build_ai_generation_prompt(background_text, selected_goals, include_optional_fields):
    goals = selected_goals if selected_goals else ["先建立 3-6 个月应急资金", "优先稳定月度现金流", "逐步压降负债规模"]

    optional_text = []
    if "货币基金/活期" in include_optional_fields:
        optional_text.append("- 货币基金/活期（可选）")
    if "股票投资/基金投资" in include_optional_fields:
        optional_text.append("- 股票投资 / 基金投资（可选）")
    if "信用卡负债/小额消费信贷" in include_optional_fields:
        optional_text.append("- 信用卡负债 / 小额消费信贷（可选）")
    if "其他资产/其他负债" in include_optional_fields:
        optional_text.append("- 其他资产 / 其他负债（可选）")
    optional_part = "\n".join(optional_text) if optional_text else "- 可选字段可全部为0"

    prompt = f"""
请基于“人物背景 + 用户勾选的目标偏好”，直接生成一个完整的 12 个月家庭财务数据，输出必须是标准 JSON，不要额外解释。

【人物背景】
{background_text.strip() if background_text.strip() else "请按普通人画像自行生成。"}
...
【输出要求】
1. 必须输出完整的 12 个月数据，从 1月 到 12月；
2. 必须输出 profile 与 monthly_data；
3. 每个月都要有 note；
4. 输出只能是 JSON；
5. JSON格式如下：
{
...
with tab4:
    ...
    st.write("这里不是单纯粘贴 JSON 了，而是可以先输入人物背景、勾选目标与字段偏好，再让 AI 按条件生成 12 个月数据。")
    ...
    prompt_preview = build_ai_generation_prompt(background_text, selected_goals, include_optional_fields)
    st.text_area("给 DeepSeek 的生成提示词", value=prompt_preview, height=360)

    c1, c2 = st.columns(2)
    with c1:
        if st.button("调用 DeepSeek 直接生成12个月数据"):
            try:
                raw = call_deepseek(
                    prompt_preview,
                    system_prompt="你是一名擅长家庭财务月度建模的助手，只输出标准 JSON。"
                )
                st.session_state["generated_json_text"] = raw
                st.success("AI 已返回结果，下面会自动显示，可直接导入。")
            except Exception as e:
                st.error(f"生成失败：{e}")
    ...
    generated_json_text = st.text_area(
        "AI返回的 JSON（可修改后再导入）",
