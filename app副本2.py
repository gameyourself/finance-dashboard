...
def tolerant_json_loads(text: str):
    raw = (text or "").strip()
    if not raw:
        raise ValueError("输入框为空，请粘贴 DeepSeek 返回的完整 JSON。")
    ...
    # 如果用户粘贴的是 monthly_data 里的一个对象片段，自动补成完整结构
    if '"month"' in raw and ('"assets"' in raw or '"liabilities"' in raw):
        snippet = raw
        ...
        try:
            maybe["monthly_data"] = json.loads("[" + snippet + "]")
            return maybe
        except Exception:
            pass

    raise ValueError("不是有效 JSON。请粘贴完整 JSON，最外层要包含 profile 和 monthly_data。")
...
def import_ai_data(user, ai_json):
    ...
    for m in monthly_data:
        ...
        if month in month_map:
            ...
            assets = normalize_numeric_dict(m.get("assets", {}))
            liabilities = normalize_numeric_dict(m.get("liabilities", {}))
            for k, v in assets.items():
                if k in month_map[month]["assets"]:
                    month_map[month]["assets"][k] = v
            for k, v in liabilities.items():
                if k in month_map[month]["liabilities"]:
                    month_map[month]["liabilities"][k] = v
...
def call_deepseek(prompt: str):
    ...
    try:
        ...
        raise ValueError("DeepSeek 返回内容不是可解析 JSON，请检查提示词。")
...
def diff_from_previous(current, previous):
    ...
    for group, items in ASSET_STRUCTURE.items():
        for item in items:
            ...
            rows.append({
                "类型": group, "项目": item, "本月": cur, "上月": pre,
                "差额": cur - pre, "方向": "上升" if cur > pre else ("下降" if cur < pre else "持平")
            })
    for group, items in LIABILITY_STRUCTURE.items():
        for item in items:
...
def render_compare_table(current, previous):
    ...
    df = diff_from_previous(current, previous).rename(
        columns={"本月": f"{current_month}", "上月": f"{prev_month}"}
    )
    st.caption(f"当前展示：左侧数值列为 {current_month}，右侧数值列为 {prev_month}。")
    st.dataframe(df, use_container_width=True, hide_index=True)

    styled_source = df.copy()
    value_cols = [f"{current_month}", f"{prev_month}", "差额"]

    def style_row(row):
        if row["差额"] > 0:
            return [""] * len(row.index[:-3]) + ["color: blue;"] * 3 + [""]
        if row["差额"] < 0:
            return [""] * len(row.index[:-3]) + ["color: red;"] * 3 + [""]
        return [""] * len(row.index)

    st.write("颜色说明：高于上月可理解为蓝色，低于上月可理解为红色。")
    st.dataframe(styled_source.style.apply(style_row, axis=1), use_container_width=True)
...
def render_structure_bars(user):
    ...
    chart_mode = st.selectbox("查看模式", ["资产结构", "负债结构"], key="bar_mode")
    if chart_mode == "资产结构":
...
def render_delta_lines(user):
    ...
    for group in selected_groups:
        fig.add_trace(go.Scatter(
            x=df["month"], y=df[group], mode="lines+markers+text",
            name=group,
            text=[f"{v:,.0f}" for v in df[group]],
            textposition="top center"
        ))
    fig.update_layout(xaxis_title="月份", yaxis_title="金额")
    st.plotly_chart(fig, use_container_width=True)

    st.subheader("各结构维度月度差额")
    delta_df = df.copy()
    for group in selected_groups:
        delta_df[f"{group}_较上月差额"] = delta_df[group].diff().fillna(0)

    show_cols = ["month"] + selected_groups + [f"{g}_较上月差额" for g in selected_groups]
    st.dataframe(delta_df[show_cols], use_container_width=True, hide_index=True)
...
def build_ai_prompt(background_text, selected_principles, selected_outputs, selected_goals):
    principles_map = {
        "普通人轨迹": "数值要符合普通人的财务轨迹",
        "每月变化有原因": "每月变化要有原因，比如工资入账、日常支出、节假日消费、奖金、还贷、理财转移等",
        "允许留空": "不必把所有项目都填满，保留空白是允许的",
        "年度前后连贯": "必须让年度数据前后连贯",
        "房贷缓慢下降": "房贷余额整体缓慢下降",
        "现金合理波动": "现金围绕收入和支出进行合理波动",
        "投资规模不要夸张": "若出现投资项目，规模不要夸张",
    }
    outputs_map = {
        "基础理财目标": "给出一个最基础的理财目标",
        "每月一句原因": "每个月补一句简短说明，解释该月财务变化原因",
        "完整JSON": "输出格式必须是标准 JSON",
    }
    goal_map = {
        "建立应急金": "先建立 3-6 个月应急资金",
        "控制消费贷": "控制消费贷比例",
        "稳定现金流": "优先稳定月度现金流",
        "减少负债": "逐步压降负债规模",
        "开始小额理财": "在不影响流动性的前提下开始小额理财",
    }

    selected_goal_text = [goal_map[g] for g in selected_goals] if selected_goals else ["目标由模型自主生成，但要务实。"]

    prompt = f"""
你是一名家庭资产配置顾问，请基于下列条件生成一个“普通人、合理但不夸张”的 12 个月财务样本。

【人物背景】
{background_text if background_text.strip() else "请自行生成一个贴近现实的普通人背景，包括年龄、职业、城市级别、婚姻状态、收入特点、消费特点、是否有房贷。"}
...
【生成原则】
""" + "\n".join([f"- {principles_map[p]}" for p in selected_principles]) + f"""

【理财目标偏好】
""" + "\n".join([f"- {g}" for g in selected_goal_text]) + f"""

【额外输出要求】
""" + "\n".join([f"- {outputs_map[o]}" for o in selected_outputs]) + """
...
with tab4:
    ...
    st.write("可以先勾选生成规则与目标偏好，再复制下面自动生成的提示词给 DeepSeek；拿到完整 JSON 后粘贴回来导入。")

    background_text = st.text_area("单独输入人物背景", value=user["profile"].get("background", ""), height=120)

    col_a, col_b, col_c = st.columns(3)
    with col_a:
        selected_principles = st.multiselect(
            "生成原则",
            options=["普通人轨迹", "每月变化有原因", "允许留空", "年度前后连贯", "房贷缓慢下降", "现金合理波动", "投资规模不要夸张"],
            default=["普通人轨迹", "每月变化有原因", "允许留空", "年度前后连贯", "房贷缓慢下降", "现金合理波动"]
        )
    with col_b:
        selected_outputs = st.multiselect(
            "额外输出",
            options=["基础理财目标", "每月一句原因", "完整JSON"],
            default=["基础理财目标", "每月一句原因", "完整JSON"]
        )
    with col_c:
        selected_goals = st.multiselect(
            "理财目标偏好",
            options=["建立应急金", "控制消费贷", "稳定现金流", "减少负债", "开始小额理财"],
            default=["建立应急金", "稳定现金流"]
        )

    prompt_preview = build_ai_prompt(background_text, selected_principles, selected_outputs, selected_goals)
    st.text_area("给 DeepSeek 的提示词", value=prompt_preview, height=350)

    ai_json_text = st.text_area("粘贴 DeepSeek 返回的完整 JSON", height=320)

    if st.button("导入 JSON 到当前用户"):
        try:
            ai_json = tolerant_json_loads(ai_json_text)
            ...
            save_users(users_data)
            st.success("导入成功，已保存到本地。注意：请尽量粘贴完整 JSON，而不是局部片段。")
        except Exception as e:
            st.error(f"导入失败：{e}")
